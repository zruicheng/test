    ...
    public partial class FormMain : Form
    {
        ...
        public void getTree()
        {
            try
            {
                ...
                sql = "SELECT     DeviceID, CompanyID, CarNo FROM  Car";
                dtTem = sqlRead(sql, ConnStr);
                for (int i = 0; i < dtTem.Rows.Count; i++)
                {
                ...
                lose = new int[dtD.Rows.Count];
                YJCard = new int[dtD.Rows.Count];
                comLose = new int[dtD.Rows.Count];

                comLastStr = new string[dtD.Rows.Count];
                for (int i = 0; i < comLastStr.Length; i++)
                {
                    comLastStr[i] = "";
                }

                P1 = new string[dtD.Rows.Count];
                for (int i = 0; i < P1.Length; i++)
                {
                    P1[i] = "";
                }

                P2 = new string[dtD.Rows.Count];
                for (int i = 0; i < P2.Length; i++)
                {
                    P2[i] = "";
                }

                P3 = new string[dtD.Rows.Count];
                for (int i = 0; i < P3.Length; i++)
                {
                    P3[i] = "";
                }

                P4 = new string[dtD.Rows.Count];
                for (int i = 0; i < P4.Length; i++)
                {
                    P4[i] = "";
                }

                P5 = new string[dtD.Rows.Count];
                for (int i = 0; i < P5.Length; i++)
                {
                    P5[i] = "";
                }

                P6 = new string[dtD.Rows.Count];
        ...
        private void FormMain_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            sql = "Insert into Record(DeviceID, SimNo, CarNo, Speed, lng, lat,Blng,Blat, Direction, Address, DTime, RevTime, RevStr, ComStr,P1,P2,P3,P4,P5,P6,Status)  SELECT     DeviceID, SimNo, CarNo, Speed, Lng, Lat,Blng,Blat, Direction, '', DTime, RevTime, '','',P1,P2,P3,P4,P5,P6,Status  FROM         Car ";
            sqlCmd(sql, ConnStr);
        }
                            ...
                            if (inbuf.Length >= 12)
                            {
                                ...
                                if (check == checkByte)
                                {
                                    ...
                                    if (ComRev.Length > 0 || double.Parse(Lng) > 0)
                                    {
                                        ...
                                        string sql = "Insert into Record(DeviceID, SimNo, CarNo, Speed, lng, lat,Blng,Blat, Direction, Address, DTime, RevTime, RevStr, ComStr,P1,P2,P3,P4,P5,P6,Status)  SELECT     DeviceID, SimNo, CarNo, Speed, Lng, Lat,Blng,Blat, Direction, '', DTime, RevTime, '" + RevStr + "','" + ComRev + "',P1,P2,P3,P4,P5,P6,Status  FROM         Car where DeviceID ='" + ConnNo + "'";
                                        sqlCmd(sql, ConnStr);
                                    }
        ...
        private void Translate()
        {
            ...
            {
                {
                    string sql = "select DeviceID,lng,lat from Car where BDFlag = 0 ";
                    DataTable dtTem = sqlRead(sql, ConnStr);
                    for (int i = 0; i < dtTem.Rows.Count; i++)
                    {
        ...
        public void backAddr(string deviceid,string address,double lng,double lat) {
            if (address.Length > 0)
            {
                ...
                if (address.Length == 0)
                    address = "-";
                string sql = "update Car set address='" + address + "',Blat=" + lat + ",Blng=" + lng + ",BDFlag=1 where DeviceID='" + deviceid + "'";
                sqlCmd(sql, ConnStr);
                checkCarArea(deviceid);//通过百度地图确定车辆是否在要配送的站点内
            }
        }
            ...
            try
            {
                for (int i = 0; i < lose.Length; i++)
                {
                    ...
                    if (lose[i] >= loseTime && dtD.Rows[i]["连接"].ToString() != "断开")
                    {
                        ...
                        sqlCmd(sql, ConnStr);
                        sql = "Insert into Record(DeviceID, SimNo, CarNo, Speed, lng, lat,Blng,Blat, Direction, Address, DTime, RevTime, RevStr, ComStr,P1,P2,P3,P4,P5,P6,Status)  SELECT     DeviceID, SimNo, CarNo, Speed, Lng, Lat,Blng,Blat, Direction, '', DTime, RevTime, '','',P1,P2,P3,P4,P5,P6,Status  FROM         Car  where  DeviceID='" + dtD.Rows[i]["终端号"] + "'";
                        sqlCmd(sql, ConnStr);
                        EventNo = 3;
            ...
            {
                {
                    ...
                    for (int i = 0; i < dtTem.Rows.Count; i++)
                    {
                        ...
                        if (connNo.Length > 0)
                        {
                            ...
                            string sql1 = "update DownCom set Status=1, Flag=1,SendTime='" + DateTime.Now.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss") + "' where ID=" + id;
                            sqlCmd(sql1, ConnStr);
                        }
                    }
                }
                ...
                if (Port.Length > 0)
                {
                    if (i == 0)
                    {
                        if (P1[index] != Port)
                        {
                            ...
                            string sql = "insert into RecordPorts(DeviceID, CarNo, Port, Status, Address, lng, lat,Blng,Blat, CreateTime) SELECT  DeviceID,CarNo," + (i + 1).ToString() + ",'" + Port + "',Address,Lng,Lat,Blng,Blat,'" + time + "'  FROM         Car where  DeviceID ='" + connNO + "'";
                            sqlCmd(sql, ConnStr);
                        }
                    }
                    if (i == 1)
